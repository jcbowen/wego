...
```go
// 解析解密后的数据（符合微信官方规范）<mcreference link="https://developers.weixin.qq.com/doc/oplatform/Third-party_Platforms/2.0/api/Before_Develop/Technical_Plan.html" index="1">1</mcreference>
func parseDecryptedData(decrypted []byte) (random string, msgLen int, msg []byte, appid string, err error) {
    if len(decrypted) < 20 {
    ...
    // 提取16位随机字符串
    random = string(decrypted[:16])
    
    // 解析消息长度（4字节网络字节序）
    msgLen = networkBytesOrderToInt(decrypted[16:20])
