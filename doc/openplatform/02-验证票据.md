...
```go
// 处理票据推送消息
func (client *OpenPlatformClient) HandleVerifyTicket(ctx context.Context, msg *Message) error {
    if msg.InfoType != "component_verify_ticket" {
        return fmt.Errorf("不是票据推送消息")
    }
    
    // 存储票据
    err := client.storage.SaveVerifyTicket(ctx, msg.ComponentVerifyTicket)
    if err != nil {
        return fmt.Errorf("存储票据失败: %v", err)
    }
...
```go
// 获取当前票据
func (client *OpenPlatformClient) GetVerifyTicket(ctx context.Context) (string, error) {
...
func (client *OpenPlatformClient) GetComponentAccessToken(ctx context.Context) (string, int, error) {
    ticket, err := client.GetVerifyTicket(ctx)
    if err != nil {
...
func main() {
    ...
    http.HandleFunc("/wego/callback", func(w http.ResponseWriter, r *http.Request) {
        ...
        case "component_verify_ticket":
            // 处理票据推送
            err := client.HandleVerifyTicket(ctx, msg)
            if err != nil {
